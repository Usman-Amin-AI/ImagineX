...
def worker():
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras, persist_image=True) -> list:
        ...
        for x in imgs:
            ...
            d.append(('Version', 'version', 'ImagineX v' + fooocus_version.version))
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        if use_expansion:
            ...
            for i, t in enumerate(tasks):
                ...
                progressbar(async_task, current_progress, f'Preparing ImagineX text #{i + 1} ...')
...
# Helper: create a minimal AsyncTask from structured params and enqueue it.
def create_task_from_params(params: dict):
    import numpy as _np
    from modules.flags import Performance
    ...
    t.prompt = params.get('prompt', '')
    t.negative_prompt = params.get('negative_prompt', '')
    t.style_selections = params.get('styles', [])
    perf = params.get('performance', 'SPEED')
    try:
        t.performance_selection = Performance[perf] if isinstance(perf, str) and perf in Performance.__members__ else Performance.SPEED
    except Exception:
        t.performance_selection = Performance.SPEED
    t.steps = params.get('steps', t.performance_selection.steps())
    t.original_steps = t.steps
    t.aspect_ratios_selection = params.get('aspect_ratios_selection', [])
    t.image_number = params.get('num_images', 1)
    t.output_format = params.get('output_format', 'png')
    t.seed = int(params.get('seed', -1))
    t.read_wildcards_in_order = False
    t.sharpness = params.get('sharpness', 0.0)
    t.cfg_scale = params.get('cfg_scale', 7.0)
    t.base_model_name = params.get('base_model_name', modules.config.default_base_model_name)
    t.refiner_model_name = params.get('refiner_model_name', modules.config.default_refiner_model_name)
    t.refiner_switch = params.get('refiner_switch', modules.config.default_refiner_switch)
    t.loras = params.get('loras', [])
    t.input_image_checkbox = params.get('input_image_checkbox', False)
    t.current_tab = params.get('current_tab', 'uov')
    t.uov_method = params.get('uov_method', modules.flags.disabled)
    t.uov_input_image = params.get('uov_input_image', None)
    t.outpaint_selections = params.get('outpaint_selections', [])

    # Inpaint specific
    t.inpaint_input_image = params.get('inpaint_input_image', None)
    t.inpaint_additional_prompt = params.get('inpaint_additional_prompt', '')
    t.inpaint_mask_image_upload = params.get('inpaint_mask_image_upload', None)
    t.inpaint_disable_initial_latent = params.get('inpaint_disable_initial_latent', False)
    t.inpaint_engine = params.get('inpaint_engine', modules.config.default_inpaint_engine_version)
    t.inpaint_strength = params.get('inpaint_strength', 1.0)
    t.inpaint_respective_field = params.get('inpaint_respective_field', 0.618)

    t.disable_preview = params.get('disable_preview', True)
    t.disable_intermediate_results = params.get('disable_intermediate_results', True)
    t.disable_seed_increment = params.get('disable_seed_increment', False)
    t.black_out_nsfw = params.get('black_out_nsfw', False)

    t.adm_scaler_positive = params.get('adm_scaler_positive', 1.0)
    t.adm_scaler_negative = params.get('adm_scaler_negative', 1.0)
    t.adm_scaler_end = params.get('adm_scaler_end', 0.0)
    t.adaptive_cfg = params.get('adaptive_cfg', modules.config.default_cfg_tsnr)
    t.clip_skip = params.get('clip_skip', modules.config.default_clip_skip)
    t.sampler_name = params.get('sampler_name', modules.config.default_sampler)
    t.scheduler_name = params.get('scheduler_name', modules.config.default_scheduler)
    t.vae_name = params.get('vae_name', modules.config.default_vae)

    # Enhance / upscale controls are omitted in minimal API
    # Set processing flags
    t.processing = False
    t.yields = []
    t.results = []

    # If provided, convert base64 images to numpy arrays (API should pass already-decoded images)
    try:
        if isinstance(t.inpaint_input_image, dict) and 'image' in t.inpaint_input_image:
            pass
    except Exception:
        pass

...
# Backend selection for future backbone flexibility. Default remains SDXL.
default_backend = get_config_item_or_set_default(
    key='default_backend',
    default_value='sdxl',
    validator=lambda x: isinstance(x, str) and x.lower() in {'sdxl', 'flux'},
    expected_type=str
)
backends.set_backend(default_backend)
...
# Optional safety moderation stage for shared/public deployments; off by default.
default_enable_safety_checker = get_config_item_or_set_default(
    key='default_enable_safety_checker',
    default_value=False,
    validator=lambda x: isinstance(x, bool),
    expected_type=bool
)
